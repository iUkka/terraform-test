...
resource "vsphere_virtual_machine" "terraform-ubuntu" {
  # VM placement #
  count            = "${length(var.vsphere_ipv4_address)}"
  name             = "0${count.index}-${var.vsphere_vm_name}.${var.vsphere_domain}"
  ...
  # VM storage #
  disk {
    label            = "0${count.index}-${var.vsphere_vm_name}.${var.vsphere_domain}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "0${count.index}-${var.vsphere_vm_name}"
        domain    = "${var.vsphere_domain}"
        time_zone = "${var.vsphere_time_zone}"
      }

      network_interface {
        ipv4_address = "${var.vsphere_ipv4_address[count.index]}"
